  ...
  static parseFocusAreasFile(filePath: string): FocusArea[] {
    try {
      ...
      if (!Array.isArray(focusAreas)) {
        throw new Error(
          'Focus areas file must contain an array of focus area objects'
        );
      }
      ...
      for (let i = 0; i < focusAreas.length; i++) {
        ...
        if (!area.color || typeof area.color !== 'string') {
          throw new Error(
            `Focus area "${area.name}" must have a "color" field (got: ${JSON.stringify(area.color)})`
          );
        }
        if (!ColorUtils.isValidColor(area.color)) {
          throw new Error(
            `Focus area "${area.name}" has invalid color "${area.color}". Use hex colors (#FF0000), named colors (red, pink, blue), or RGB format (rgb(255,0,0))`
          );
        }
        if (!Array.isArray(area.areas)) {
          throw new Error(
            `Focus area "${area.name}" must have an "areas" array`
          );
        }
        if (area.url && typeof area.url !== 'string') {
          throw new Error(
            `Focus area "${area.name}" url field must be a string if provided`
          );
        }
      }
      ...
      if (error instanceof Error) {
        throw new Error(
          `Failed to parse focus areas file "${filePath}": ${error.message}`
        );
      }
      throw new Error(
        `Failed to parse focus areas file "${filePath}": Unknown error`
      );
    }
  }

  static getEntitiesForFocusArea(
